...
def draw_tracks(frame, tracks, show_flow=False, show_cov=False):
    for track in tracks:
        ...
        # draw_bbox(frame, track.tlbr, get_color(track.trk_id), 2, str(LABEL_MAP[track.label]))
        if show_flow:
...
def draw_bbox(frame, tlbr, color, thickness, text=None):
    ...
    # # HeatMap
    # try:
    #     mask = frame[tlbr[1]:tlbr[3], tlbr[0]:tlbr[2]]
    #     crop_mapped = cv2.applyColorMap(mask, cv2.COLORMAP_JET)
    #     fin = cv2.addWeighted(crop_mapped, 0.5, mask, 0.5, 0)
    #     frame[tlbr[1]:tlbr[3], tlbr[0]:tlbr[2]] = fin
    # except:
    #     pass
